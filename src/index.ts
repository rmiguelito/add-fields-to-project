...
class SingleSelectField extends Field {
  constructor(
    ...
    projectId: string,
    itemId: string,
    private optionMap: { [statusName: string]: string }
  ) {
  ...
  setValue(value: string): void {
    ...
    if (!optionId) {
      throw new Error(
        `Unknown option '${value}' for field '${this.fieldName}'. Available options: ${Object.keys(this.optionMap).join(', ')}`
      );
    }
...
class FieldManager {
  ...
  private fieldConfig: ProjectFieldConfig;

  constructor(private projectId: string, private itemId: string) {
    this.fieldConfig = FIELD_CONFIGS[projectId] || FIELD_CONFIGS.default;
  }

  registerField(fieldName: string, value: string | undefined): void {
    if (!value || value.trim() === '') {
      return;
    }
  ...
  applyAll(): void {
    ...
    this.fields.forEach((field, fieldName) => {
      try {
        ...
        throw new Error(
          `Failed to update field '${fieldName}': ${(error as Error).message}`
        );
      }
    });
  }
...
class ProjectItemManager {
  ...
  getFieldManager(itemId: string): FieldManager {
